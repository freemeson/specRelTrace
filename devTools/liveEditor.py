...
class ShaderViewer(FloatLayout):
    ...
    def update_shader(self, *args):
        ...
        if self.size[0] == 0:
            ...
            s['screen_ratio'] = self.size[1]/self.size[0]
        s.ask_update()

    def on_touch_move(self, touch):
        
        self.phi += 3*touch.dx/self.size[0]
        self.psy += 3*touch.dy/self.size[1]
        self.psy = max(-0.1, min(0.9, self.psy))
        
        self.canvas['phi'] = self.phi
        self.canvas['psy'] = self.psy

    
    def _on_resize(self, width, height):
        print( heigth/width)
        self.canvas['screen_ratio'] = heigth/width
